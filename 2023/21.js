export default function (input) {
    ...
    const getTargets = (start, targetSteps) => {
        let steps = 0, nodes = [start], visited = [], targets = []
        while (steps <= targetSteps) {
            ...
            nodes = newNodes
                ...
                .filter((e,i,a) => a.findIndex(n => n.x === e.x && n.y === e.y) === i)
            steps++
        }
        return targets
    }
    ...
    let output = BLOCKS**2 * oddSuperBlock.length + (BLOCKS+1)**2 * evenSuperBlock.length

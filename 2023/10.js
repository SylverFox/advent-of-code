export default function (input) {
  ...
  const getReachableNeighbours = ({ x, y }) => {
    ...
    if (
      y > 0 &&
      (exits & 0b1000) > 0 &&
      (EXITS.indexOf(input[y - 1][x]) & 0b0010) > 0
    ) {
    ...
    if (
      x < WIDTH - 1 &&
      (exits & 0b0100) > 0 &&
      (EXITS.indexOf(input[y][x + 1]) & 0b0001) > 0
    ) {
    ...
    if (
      y < HEIGHT - 1 &&
      (exits & 0b0010) > 0 &&
      (EXITS.indexOf(input[y + 1][x]) & 0b1000) > 0
    ) {
      neighbours.push({ x, y: y + 1 })
    }
    // left
    if (
      x > 0 &&
      (exits & 0b0001) > 0 &&
      (EXITS.indexOf(input[y][x - 1]) & 0b0100) > 0
    ) {
      neighbours.push({ x: x - 1, y })
    }
  ...
  while (nodes.length) {
    ...
    while (nodes.length) {
      ...
      const neighbours = getReachableNeighbours(next).filter(
        (n) => !visited.includes(`${n.x}.${n.y}`)
      )

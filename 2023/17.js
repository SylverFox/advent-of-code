...
function part1(input) {
    ...
    const getDirection = (p1, p2) => p1.x < p2.x ? '>' : p1.x > p2.x ? '<' : p1.y > p2.y ? '^' : 'v'
    const getDirectionPattern = (path) => path.reduce((a,v,i,p) => i === 0 ? '' : a + getDirection(p[i-1], v), '')

    const nodeCache = {}
    let paths = [[{x: 0, y: 0}]]
    let maxCost = input[0].slice(1).sum() + input.slice(1).map((i) => i[i.length - 1]).sum()
    while(paths.length) {
        const newPaths = []
        ...
        branch: while(paths.length) {
            ...
            // cost for this path
            const cost = path.slice(1).map(p => input[p.y][p.x]).sum()
            ...
            const approachVector = path.length < 2 ? '' : getDirectionPattern(path.slice(-4))
                .match(/(.)\1*$/)[0] // Match only the last repeating characters
            ...
            for (let dir of directions) {
                // out of bounds
                if (dir.x < 0 || dir.y < 0 || dir.x >= WIDTH || dir.y >= HEIGHT) continue
...
function part2(input) {
    ...
    main: while(paths.length) {
        ...
        const approachVector = path.length === 0 ? '' : path.slice(-10).match(/(.)\1*$/)[0] // Match only the last repeating characters

        // out of bounds
        if (x < 0 || y < 0 || x >= WIDTH || y >= HEIGHT) continue
        // no loops or reversal
        const uniqueCoordinates = coordinates.map(c => `${c.x},${c.y}`).filter((c,i,a) => i === a.indexOf(c))
        if (uniqueCoordinates.length !== coordinates.length) continue

        // cost for this path
        const cost = coordinates.slice(1).map(c => input[c.y][c.x]).sum()
        ...
        for (let c = 1; c < coordinates.length - 1; c++) {
            ...
            const cost = coordinates.slice(1, c + 1).map(p => input[p.y][p.x]).sum()

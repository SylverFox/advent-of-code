export default function (input) {
    ...
    const validPassports = passports
        .map(p => p.map(l => l.split(':')[0]).sort().join(''))
        .filter(p => p === 'byrcidecleyrhclhgtiyrpid' || p === 'byrecleyrhclhgtiyrpid')
    ...
    const validateFields = passport => ['byrcidecleyrhclhgtiyrpid', 'byrecleyrhclhgtiyrpid'].includes(Object.keys(passport).sort().join(''))
    ...
    const validPassports2 = passports
        ...
        .filter(p => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(p.ecl))

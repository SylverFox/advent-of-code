export default function (input) {
    input = input.map(l => l.split(' -> ').map(c => c.split(',').map(Number)))
    ...
    const output = Object.keys(coords).filter(c => coords[c] > 1).length
    console.log('Part 1:', output)

    coords = {}

    function drawLine2(line) {
        let min = 0, max = 0;
        if (line.a.x === line.b.x) {
            // vertical
            min = Math.min(line.a.y, line.b.y);
    ...
    console.log('Part 2:', Object.keys(coords).filter(c => coords[c] > 1).length)
}

export default function (input) {
    ...
    const {X, Y, Z} = input.reduce((a,v) => [...a, v[0], v[1]], []).reduce((a,v) => ({
        X: Math.max(a.X, v[0] + 1),
        Y: Math.max(a.Y, v[1] + 1),
        Z: Math.max(a.Z, v[2])
    }), {X: 0, Y: 0, Z: 0})

    let map = [...Array(Z)].map(() => [...Array(Y)].map(() => [...Array(X)].fill('.')))

    let blockSupports = {}
    for (let i = 0; i < input.length; i++) {
        const block = input[i]
        let [x, y, z] = block[0], [xmax, ymax, zmax] = block[block.length -1]
        const zmin = z
        ...
        for (let level = zmin; level >= 0; level--) {
            const fits = level > 0 && chunks.every(chunk => map[level - 1][chunk[1]][chunk[0]] === '.')
            if (!fits) {
                // block doesn't fit on next level, place here
                chunks.forEach(chunk =>
                    map[level + (chunk[2] - zmin)][chunk[1]][chunk[0]] = String.fromCharCode(65 + i)
                )
                // and tag supporting blocks
                if (level > 0) {
                    const supports = chunks.map(chunk => map[level - 1][chunk[1]][chunk[0]])
    ...
    for (let block in blockSupports) {
        if (!allSupports.includes(block)) {
            ...
            let supports = Object.keys(blockSupports)
                .filter(bs => blockSupports[bs].length === 1 && blockSupports[bs][0] === block)
            ...
            if (!supports.length) {
                ...
                while(supports.length) {
                    ...
                    supports = Object.keys(blockSupports)
                        .filter(bs => !falling.includes(bs) && blockSupports[bs].every(s => falling.includes(s)))
                }

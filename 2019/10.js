...
export default function(input) {
    ...
    for (let y = 0; y < map.length; y++) {
        for (let x = 0; x < map[0].length; x++) {
            if (map[y][x] === '#') {
                const angle = ((Math.atan2(position.y - y, position.x - x)*180 / Math.PI + 180 + 90) % 360)
                const distance = Math.sqrt((position.y - y)**2 + (position.x - x)**2)
                pointData.push({
                    angle, distance, position: {x, y}
                })
            }
        }
    }
    pointData.sort((a,b) => a.angle !== b.angle ? a.angle - b.angle : a.distance - b.distance)

    let lastAngle = -1, vaporized = 0, lastAsteroidHit
    while (vaporized < 200) {

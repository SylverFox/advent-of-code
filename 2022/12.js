export default function (input) {
    ...
    let nodes = [{ pos: start, dist: 0 }], visited = [{ pos: start, dist: 0 }]
    main: while (nodes.length) {
        ...
        const neighbours = [{x: -1, y: 0}, {x: 1, y: 0}, {x: 0, y: -1}, {x: 0, y: 1}]
            ...
            .filter(n => n.x >= 0 && n.y >= 0 && n.y < input.length && n.x < input[0].length)
        ...
        for (let neighbour of neighbours) {
            ...
            const visit = visited.find(n => n.pos.x === neighbour.x && n.pos.y === neighbour.y)
            if (visit && visit.dist > next.dist + 1) {
                visit.dist = next.dist + 1
            } else if (!visit) {
                visited.push({ pos: {x: neighbour.x, y: neighbour.y}, dist: next.dist + 1 })
                nodes.push({ pos: {x: neighbour.x, y: neighbour.y}, dist: next.dist + 1 })
            }
        }
    }

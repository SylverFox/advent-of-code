...
export default function (input) {
    ...
    let password = '', password2 = Array(8).fill('.')
    for (let i = 0; ; i++) {
        const hash = createHash('md5').update(input + i).digest().toString('hex')
        if (hash.startsWith('00000')) {
            if (password.length < 8) password += hash[5]
            if (parseInt(hash[5], 16) < 8 && password2[hash[5]/1] === '.') password2[hash[5]/1] = hash[6]

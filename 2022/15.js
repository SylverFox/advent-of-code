export default function (input) {
  ...
  const getCoveredRanges = (target) => {
    ...
    for (let i of input) {
      ...
      if (Math.abs(dY) <= dist) {
        ...
        const range = coveredRanges.findIndex(
          (r) => xMin <= r.xMax && xMax >= r.xMin
        )
        if (range !== -1) {
    ...
    for (let range of coveredRanges) {
      const overlap = outputRanges.find(
        (or) => range.xMin <= or.xMax && range.xMax >= or.xMin
      )
      if (!overlap) {
  ...
  let output = getCoveredRanges(Y_TARGET)
    .map((r) => r.xMax - r.xMin)
    .reduce((a, b) => a + b)
  ...
  let parsed = input
    ...
    .map((i) => ({
      ...
      xB: i[2] / 1,
      yB: [3] / 1,
      r: Math.abs(i[3] - i[1]) + Math.abs(i[2] - i[0]),
    }))
  ...
  main: for (let y = 0; y < MAX_COORDINATE; y++) {
    ...
    let rangeMin = 0,
      rangeMax = MAX_COORDINATE
    while (ranges.length) {
      const r = ranges.findIndex(
        (r) => r.xmin <= rangeMin || r.xmax >= rangeMax
      )

export default function (input) {
    ...
    let position = { x: startX, y: 0 }, direction = 0
    for (let instruction of instructions) {
        if (instruction === 'R') direction = (direction + 1) % 4
        else if (instruction === 'L') direction = (direction + 3) % 4
        else for (let _ in Array(instruction/1).fill()) position = nextPos(position, direction)
    }
    console.log('Part 1:', 1000 * (position.y + 1) + 4 * (position.x + 1) + direction)
    ...
    for (let instruction of instructions) {
        ...
        else for (let _ in Array(instruction/1).fill()) position = nextPos2(position, direction)
    }
    console.log('Part 2:', 1000 * (position.y + 1) + 4 * (position.x + 1) + direction)

export default function (input) {
    ...
    for (; i < input.length; i++) {
        ...
        for (let n of nodes.split(',')) {
            if (n.includes(':')) {
                const [_, param, op, value, next] = n.match(/^([xmas])([<>])(\d+):(\w+)$/)
                rules.push({
                    f: (obj) => op === '<' ? obj[param] < value/1 : obj[param] > value/1,
                    n: next
                })
            } else if (n === 'A') {
                rules.push({ f: () => true, n })
    ...
    for (i++; i < input.length; i++) {
        const values = input[i].slice(1, -1).split(',')
            .map(v => v.split('='))
    ...
    let output = parts.filter(checkPart).map(p => Object.values(p).sum()).sum()
    ...
    for (let i = 0; input[i] !== ''; i++) {
        ...
        const instruction = instr.split(',')
            .map(i => i.includes(':') ? i.replace(/^(.*?):/, '"$1":') : '"*":'+i)
            .join(',')
            .replace(/([AR])/g, '"$1"')
        instructions.push({
            name,
    ...
    while (instructions.length > 1) {
        ...
        let next = instructions.findIndex(i => i.name !== 'in' && !i.instr.match(nodeNames))
        next = instructions.splice(next, 1)[0]
        instructions = instructions.map(
            i => ({...i, instr: i.instr.replace(new RegExp('\\b'+next.name+'\\b'), next.instr)})
        )
    }
    ...
    while (nodes.length) {
        ...
        for (let rule of nodeRules) {
            if (rule === '*') {
                ...
                const param = rule[0], op = rule[1], value = rule.slice(2)/1
                if (next.rules[rule] === 'A') {
                    const acceptedConstraint = JSON.parse(JSON.stringify(nodeConstraint))
                    if (op === '<') {
                        acceptedConstraint[param][1] = Math.min(acceptedConstraint[param][1], value - 1)
                        nodeConstraint[param][0] = Math.max(nodeConstraint[param][0], value)
                    } else {
                        acceptedConstraint[param][0] = Math.max(acceptedConstraint[param][0], value + 1)
                    ...
                    if (op === '<') {
                        continueConstraint[param][1] = Math.min(continueConstraint[param][1], value - 1)
                        nodeConstraint[param][0] = Math.max(nodeConstraint[param][0], value)
                    } else {
                        continueConstraint[param][0] = Math.max(continueConstraint[param][0], value + 1)
                        nodeConstraint[param][1] = Math.min(nodeConstraint[param][1], value)
                    }
                    nodes.push({
                        rules: next.rules[rule],
    ...
    output = acceptedConstraints.map(c => Object.values(c).map(v => v[1] - v[0] + 1).product()).sum()

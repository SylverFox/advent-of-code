export default function (input) {
    ...
    const runSimulation = (fixedLights) => {
        for (let i = 0; i < 100; i++) {
            let copy = Array(grid.length).fill().map(r => Array(grid[0].length).fill('.'))

            for (let y = 0; y < grid.length; y++) {
                for (let x = 0; x < grid.length; x++) {
                    const lights = neighbours
                        .map(n => [n[0] + x, n[1] + y])
                        .filter(n => n[0] >= 0 && n[1] >= 0 && n[0] < grid[0].length && n[1] < grid.length)
                        .filter(n => grid[n[1]][n[0]] === '#')
                        .length
        ...
        return grid.map(row => row.join('').replace(/\./g, '').length).reduce((a, b) => a + b)
    }

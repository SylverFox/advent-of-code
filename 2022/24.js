...
export default function (input) {
    ...
    const directions = [{x: 0, y: 0}, {x: -1, y: 0}, {x: 0, y: -1}, {x: 1, y: 0}, {x: 0, y: 1}]
    const lcmSteps = lcm(map.length, map[0].length)
    let routes = [{x: startX, y: -1}], steps = 0, cache = Array(lcmSteps).fill().map(() => [])
    main: while (true) {
        let newRoutes = []
        steps++
    ...
    routes = [{x: endX, y: map.length}], cache = Array(lcmSteps).fill().map(() => [])
    main: while (true) {
        let newRoutes = []
        steps++
    ...
    routes = [{x: startX, y: -1}], cache = Array(lcmSteps).fill().map(() => [])
    main: while (true) {
        let newRoutes = []
        steps++

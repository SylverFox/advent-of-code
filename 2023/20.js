export default function (input) {
    ...
    if (i.startsWith('broadcaster')) {
      ...
      machines[name.slice(1)] = {
        ...
        f: function (name, input) {
          ...
          this.state = Object.values(this.inputs).every((i) => i)
            ? (this.state = false)
            : (this.state = true)
          return true
        },
  ...
  const press = (machines) => {
    ...
    while (nodes.length) {
      ...
      if (propagates) {
        machines[node.m].outputs.forEach((o) =>
          nodes.push({
            m: o,
            i: node.m,
  ...
  const rxGate = Object.keys(machines).find((m) =>
    machines[m].outputs.includes('rx')
  )
  const rxGateInputs = Object.keys(machines).filter((m) =>
    machines[m].outputs.includes(rxGate)
  )
